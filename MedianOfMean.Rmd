...
for (i in 1:1000) {
  ...
  n <- length(estimateur)
  sigma <- 8.22
  sup <- sigma * sqrt(6.51 * log(1 / alpha) / n) 
  inf <- - sigma * sqrt(6.51 * log(1 / alpha) / n)

  count_in_intervalle <- sum(estimateur-5.5 >= inf & estimateur-5.5 <= sup)
  pourcentage <- count_in_intervalle / length(estimateur) * 100
...
estimateur-5.5
hist(estimateur-5.5, xlim=c(inf-1, sup+1))
abline(v = sup, col = "red")
abline(v = inf, col = "red")

sum(estimateur-5.5 <= inf)

hist(pourcentages, breaks=100, freq=T, xlim = c(95,100) , ylim = c(500, 700))
plot(estimateur-5.5, type = "p", col = "blue", xlab = "Valeur de c", ylab = "MoM-mu", main = "Estimateur MoM", ylim=c(inf-1, sup+1))
sup
abline(h = sup, col = "red")
abline(h = inf, col = "red")
#estimateur
...
#print(paste("Nombre de valeurs comprises entre 96.5% et 98.5% : ", ululu))
...
ouloulou <- 0
for (i in 1:1000) {
  Z <- rweibull(1000, shape = 0.5)
  estimateur <- replicate(1000, median_of_means_opt3(rweibull(100, shape = 0.5)))

  alpha <- 0.025
  mu_MoM <- median_of_means_opt3(rweibull(100, shape = 0.5))
  n <- length(estimateur)
  sigma <- 4.69
  sup <- mu_MoM + sigma * sqrt(12.2 * log(1 / alpha) / n) 
  inf <- mu_MoM - sigma * sqrt(12.2 * log(1 / alpha) / n)

  count_in_intervalle <- sum(estimateur >= inf & estimateur <= sup)
  pourcentage <- count_in_intervalle / length(estimateur) * 100

  pourcentages[i] <- pourcentage
  if (97 <= pourcentage && pourcentage <= 98) {
    ululu <- ululu + 1
  }
  if (pourcentage > 98) {
    ouloulou <- ouloulou + 1
...
print(paste("Pourcentages stockés :", pourcentages))
print(paste("Nombre de valeurs comprises entre 96.5% et 98.5% : ", ululu))
print(paste("Nombre de valeurs supérieure à 98: ", ouloulou))
...
for (i in 1:1000) {
  ...
  estimateur <- replicate(1000, median_of_means_opt3(rexp(100, rate = 0.5)))

  alpha <- 0.025
  mu_MoM <- median_of_means_opt3(Z)
  n <- length(estimateur)
  sigma <- 2
  sup <- mu_MoM + sigma * sqrt(24.3* log(1 / alpha) / n) 
  inf <- mu_MoM - sigma * sqrt(24.3* log(1 / alpha) / n)
...
hist(pourcentages, breaks=25, freq=T, xlim = c(95,100) )
plot(estimateur, type = "p", col = "blue", xlab = "Valeur de c", ylab = "MoM", main = "Estimateur MoM", ylim=c(inf-1, sup+1))
abline(h = sup, col = "red")
abline(h = inf, col = "red")
...
for (i in 1:1000) {
  Z <- rpareto(1000, scale = 1, shape = 6)
  estimateur <- replicate(1000, median_of_means_opt3(rpareto(1000, scale = 1, shape = 6)))

  alpha <- 0.025
  mu_MoM <- median_of_means_opt3(Z)
  n <- length(estimateur)
  sigma <- 0.27
  sup <- mu_MoM + sigma * sqrt(3* log(1 / alpha) / n) 
  inf <- mu_MoM - sigma * sqrt(3* log(1 / alpha) / n)
...
for (i in 1:1000) {
  Z <- rlnorm(1000, meanlog = 0, sdlog = 2)
  estimateur <- replicate(1000, median_of_means_opt3(rlnorm(1000, meanlog  = 0, sdlog  = 2)))
...
```{r}
c_opt <- seq(from = 2, to = 7, by = 0.01)
n <- 1000
alpha <- 0.025
sigma <- 8.22
shape <- 2.2
scale <- 3
num_simulations <- 1000  # Nombre de simulations pour calculer le pourcentage

# Initialiser les vecteurs
sup <- numeric(length(c_opt))
inf <- numeric(length(c_opt))
pourcentage <- numeric(length(c_opt))

# Boucle pour calculer les intervalles de confiance et les pourcentages
for (i in 1:length(c_opt)) {
  count_in_intervalle <- 0
  
  for (j in 1:num_simulations) {
    Z <- rpareto(n, shape, scale)
    estimateur <- replicate(1000, median_of_means_opt3(Z))
    mu_MoM <- median_of_means_opt3(Z)
    
    sup[i] <- mu_MoM + sigma * sqrt(c_opt[i] * log(1 / alpha) / n) 
    inf[i] <- mu_MoM - sigma * sqrt(c_opt[i] * log(1 / alpha) / n)
    
    count_in_intervalle <- count_in_intervalle + sum(estimateur >= inf[i] & estimateur <= sup[i])
  }
  
  pourcentage[i] <- count_in_intervalle / (num_simulations * 1000) * 100
}

# Tracer les résultats
plot(c_opt, pourcentage, type = "l", col = "blue", lwd = 2,
     xlab = "Valeur de c_opt", ylab = "Pourcentage dans l'intervalle",
     main = "Pourcentage dans l'intervalle de confiance en fonction de c_opt")
abline(h = 97.5, col = "red", lty = 2, lwd = 2)
#abline(v = c_opt[which.min(abs(pourcentage-97.5))])
legend("bottomright", legend = "97.5%", col = "red", lty = 2, lwd = 2)
...
pourcentages <- numeric(1000)
c_opt <- seq(from = 2, to = 7, by = 0.01)
mat <- matrix(NULL, ncol = 1001, nrow = length(c_opt))
ululu <- 0
for (j in 1:length(c_opt)){
  for (i in 1:1000) {
    mat[j,1] <- c_opt[j]
    Z <- rpareto(1000, shape = 2.2, scale = 3)
    mat[j,i+1] <-  median_of_means_opt3(rpareto(100, shape = 2.2, scale = 3))
    alpha <- 0.025
    mu_MoM <- median_of_means_opt3(Z)
    n <- 1000
    sigma <- 8.22
    sup <- mu_MoM + sigma * sqrt(c_opt[j] * log(1 / alpha) / n) 
    inf <- mu_MoM - sigma * sqrt(c_opt[j] * log(1 / alpha) / n)
    
    count_in_intervalle <- sum(mat[j,-1] >= inf & mat[j,-1] <= sup)
    pourcentage <- count_in_intervalle / 10
...
hist(pourcentages, breaks=25, freq=T, xlim = c(95,100) , ylim = c(500, 700))
plot(estimateur, type = "p", col = "blue", xlab = "Valeur de c", ylab = "MoM", main = "Estimateur MoM", ylim=c(inf-1, sup+1))
abline(h = sup, col = "red")
abline(h = inf, col = "red")
...
```{r}
time <- Sys.time()
c_opt <- seq(from = 2.5, to = 3, by = 0.005)
mat <- matrix(NA, ncol = 1001, nrow = length(c_opt))  # Matrice pour stocker les résultats
pourcentages <- numeric(length(c_opt))  # Vecteur pour stocker les pourcentages
ululu <- 0  # Compteur pour les pourcentages dans l'intervalle souhaité

# Remplissage de la première colonne de mat avec les valeurs de c_opt
mat[, 1] <- c_opt

# Boucle pour remplir la matrice et calculer les pourcentages
for (j in 1:length(c_opt)) {
  for (i in 1:1000) {
    Z <- rpareto(1000, shape = 2.2, scale = 3)
    mat[j, i + 1] <- median_of_means_opt3(rpareto(100, shape = 2.2, scale = 3))
  ...
  count_in_intervalle <- sum(mat[j, -1] >= inf & mat[j, -1] <= sup)
  pourcentage <- count_in_intervalle / 1000 * 100
  
  pourcentages[j] <- pourcentage
...
# Tracer l'histogramme des pourcentages
hist(pourcentages, breaks = 25, freq = TRUE, xlim=c(90,100))

# Tracer le pourcentage en fonction des valeurs de c_opt
plot(c_opt, pourcentages, type = "p", col = "blue", xlab = "Valeur de c", ylab = "Pourcentage", main = "Pourcentage en fonction de c", ylm= c(90,100))
abline(h = 97.5, col = "red", lty = 2, lwd = 2)
legend("bottomright", legend = "97.5%", col = "red", lty = 2, lwd = 2)

abline(v = c_opt[which.min(abs(pourcentages-97.5))], col= "green", lty = 2, lwd = 2)
legend("bottomright", legend = c("97.5%","c_optimal"), col = c("red","green"), lty = c(2,2), lwd = c(2,2))

# Affichage des résultats
print(paste("Pourcentages stockés :", paste(pourcentages, collapse = "%, ")))
print(paste("Nombre de valeurs comprises entre 96.5% et 98.5% : ", ululu))

#dev.copy(pdf, filename = "c_opt_pareto.pdf")
#dev.off()
Sys.time()- time
```
...
for (j in 1:length(c_opt)) {
  for (i in 1:1000) {
    Z <- rlnorm(1000, meanlog  = 0, sdlog  = 2)
    mat[j, i + 1] <- median_of_means_opt3(rlnorm(1000, meanlog  = 0, sdlog  = 2))
  }
  
  alpha <- 0.025
  mu_MoM <- median_of_means_opt3(rlnorm(1000, meanlog  = 0, sdlog  = 2))
  n <- 1000
  sigma <- 54.1
  sup <- mu_MoM + sigma * sqrt(c_opt[j] * log(1 / alpha) / n)
  inf <- mu_MoM - sigma * sqrt(c_opt[j] * log(1 / alpha) / n)
...
# Tracer le pourcentage en fonction des valeurs de c_opt
plot(c_opt, pourcentages, type = "l", col = "blue", xlab = "Valeur de c", ylab = "Pourcentage", main = "Pourcentage en fonction de c")
abline(h = 97.5, col = "red", lty = 2, lwd = 2)
legend("bottomright", legend = "97.5%", col = "red", lty = 2, lwd = 2)

#abline(v = c_opt[which.min(pourcentages-97.5)], col= "green", lty = 2, lwd = 2)
#legend("bottomright", legend = c("97.5%","c_optimal"), col = c("red","green"), lty = c(2,2), lwd = c(2,2))
...
dev.copy(pdf, filename = "c_opt.pdf")
dev.off()
Sys.time()- time
```
...
```{r}
time <- Sys.time()
c_opt <- seq(from = 15, to = 25, by = 0.05)
mat <- matrix(NA, ncol = 1001, nrow = length(c_opt))  # Matrice pour stocker les résultats
pourcentages <- numeric(length(c_opt))  # Vecteur pour stocker les pourcentages
ululu <- 0  # Compteur pour les pourcentages dans l'intervalle souhaité
...
for (j in 1:length(c_opt)) {
  for (i in 1:1000) {
    Z <- rexp(100, rate = 0.5)
    mat[j, i + 1] <- median_of_means_opt3(rexp(100, rate = 0.5))
  }
  
  alpha <- 0.025
  mu_MoM <- median_of_means_opt3(rexp(100, rate = 0.5))
  n <- 1000
  sigma <- 2
  sup <- mu_MoM + sigma * sqrt(c_opt[j] * log(1 / alpha) / n)
  inf <- mu_MoM - sigma * sqrt(c_opt[j] * log(1 / alpha) / n)
...
# Tracer le pourcentage en fonction des valeurs de c_opt
plot(c_opt, pourcentages, type = "p", col = "blue", xlab = "Valeur de c", ylab = "Pourcentage", main = "Pourcentage en fonction de c", ylim=c(90,100))
abline(h = 97.5, col = "red", lty = 2, lwd = 2)
legend("bottomright", legend = "97.5%", col = "red", lty = 2, lwd = 2)
...
print(paste("Pourcentages stockés :", paste(pourcentages, collapse = "%, ")))
print(paste("Nombre de valeurs comprises entre 96.5% et 98.5% : ", ululu))
print(paste("Valeur de c_opt : ", c_opt[which.min(abs(pourcentages-97.5))]))

#dev.copy(pdf, filename = "c_opt_exp.pdf")
#dev.off()
Sys.time()- time
```