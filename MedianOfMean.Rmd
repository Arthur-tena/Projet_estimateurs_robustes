...
for (i in 1:100) {
  ...
  n <- length(estimateur)
  sigma <- 8.22
  sup <- mu_MoM + sigma * sqrt(3.41 * log(1 / alpha) / n) 
  inf <- mu_MoM - sigma * sqrt(3.41 * log(1 / alpha) / n)

  count_in_intervalle <- sum(estimateur >= inf & estimateur <= sup)
  pourcentage <- count_in_intervalle / length(estimateur) * 100

  pourcentages[i] <- pourcentage
  ululu=sum(pourcentage==97.5)
  ouloulou=sum(pourcentage>97.5)
}

hist(pourcentages, breaks=20, freq=T)

print(paste("Pourcentages stockés :", pourcentages))
print(paste("Nombre de poucentage égale à 97.5 : ", ululu))
print(paste("Nombre de poucentage supérieur à 97.5 : ", ouloulou))
