...
```{r}
median_of_means_opt <- function(seq, alpha=0.025) {
  n=length(seq)
...
rob_median_of_means_opt <- function(seq, alpha=0.025, l) {
...
```{r}
median_of_means_opt2 <- function(alpha) {
...
median_of_means_opt3 <- function(seq, alpha, i) {
  n <- length(seq)
...
rob_median_of_means_opt <- function(X, alpha=0.025, l) {
  res <- replicate(l, median_of_means_opt2(X, alpha))
  return(mean(res))
}
...
plot_intervalle <- function(X) {
  ...
  for (i in 1:10000) {
    ...
    Y[i, ] <- c(y_min, y_max, taille)
  }
  print(Y)
  plot(Y[, 1], type = "l", ylim = range(Y), col = "blue", main = "Intervalles de confiance", xlab = "Index", ylab = "Valeurs")
  points(Y[, 2], type = "l", col="blue")
