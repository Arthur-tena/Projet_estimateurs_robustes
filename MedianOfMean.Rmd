...
ggplot(data = data, aes(x = i_values, y = median_values)) +
  ...
  labs(x = "c", y = "Median of Means", title = "Estimateur MoM d'une loi exp(2)") +
...
plot_intervalle <- function(X) {
  ...
  for (i in 1:10000) {
    ...
    Y[i, ] <- c(hat_mu,y_min, y_max, taille, abs(hat_mu-1))
  }
  ...
  print(Y)
  print(sigma*sqrt((32*log(1/alpha))/n))
  print(sum(Y[, 5] <=sigma*sqrt((32*log(1/alpha))/n)))
}
X1=rexp(n = 1000, rate=1)
X2=rweibull(n = 1000, shape = 0.5)
plot_intervalle(X1)
plot_intervalle(X2)
```
...
median_of_means_Minsker <- function(X, k) {
  ...
  indic <- combn(X, trunc(n/k))
  moyennes <- apply(indic, 2, mean)
  return(median(moyennes))
}

X4 <- rexp(n = 20, rate = 1/2)
result <- median_of_means_Minsker(X4, 3)
print(result)
