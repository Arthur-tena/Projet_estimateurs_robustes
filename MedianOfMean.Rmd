...
* Loi de Student à k=5 degré de liberté 
```{r}
...
* sample de valeures entières comprises entre -2 et 10
```{r}
...
* sample de valeures réelles comprises entre -1 et 1 
```{r}
population_rd2=sample(x = seq(-1,1, by=0.001), size = 1000, replace = TRUE, prob=NULL)
c(median_of_means(population_rd2, 10),mean(population_rd2),(mean(population_rd2) - median_of_means(population_rd2, 10)) ^ 2)
```

```{r}
c(rob_median_of_means(population_rd2, 10, 20),mean(population_rd2),(mean(population_rd2) - rob_median_of_means(population_rd2, 10, 20)) ^ 2)
```
