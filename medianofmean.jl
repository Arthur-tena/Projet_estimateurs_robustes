...
# Fonction qui répéte le median of mean plusieurs fois (estimateur naif)
...
# Intervalle de confiance avec bootstrap
...
median_of_means_opt(data,0.025)
...
# Tracer le graphique
plot(1:40, median_of_means_optC(data,0.025), xlabel="Valeur de c", ylabel="Estimateur (médiane des moyennes)", 
    title="Estimateur en fonction de la valeur de c", legend=false)


function median_of_means_opt_C2(seq, alphas=0.001:0.01:0.15)
    medians = []
    for α in alphas
        if α <= 0 || α > 1
            throw(ArgumentError("alpha doit être supérieur à 0 et inférieur ou égal à 1"))
